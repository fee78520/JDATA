...
def get_train():
    ...
    # #输出结果
    # # result.to_csv(r'../time/time_3_in358.csv', index=None)
    # result[['label_1','label_2']] = test[['label_1','label_2']]
    # s1,s2=score(result)

    train = pd.read_csv(r'../input/vali_train_s1.csv')
    test = pd.read_csv(r'../input/vali_test_s1.csv')
    train['label_1'] = [1 if x>0 else 0 for x in train['label_1']]
    test['label_1'] = [1 if x>0 else 0 for x in test['label_1']]
    result = test[['user_id']]#要提交的结果
    #训练S1
    ...
    result['pred_date'], feat_imp_s2= fit_predict(X,y,X_pred)
    #输出结果
    result.to_csv(r'../time/time_4_in358.csv', index=None)
    result[['label_1','label_2']] = test[['label_1','label_2']]
    s3,s4=score(result)
...
def get_result():
    train = pd.read_csv(r'../input/test_train_s1.csv')
    test = pd.read_csv(r'../input/test_test_s1.csv')
    drop_column = ['user_id','label_1','label_2']
    ...
    sub.loc[:49999,:].to_csv(r'../result/sub_all.csv',index=None,encoding='utf-8')
