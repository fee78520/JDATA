...
from util import feat_nunique, feat_sum, feat_count, feat_max, load_data, feat_mean, feat_min, feat_median, last_day, \
    action_last, guanxing
...
def create_feat(start_date, end_date, order, action, test=False):
    ...
    order_tmp = order_tmp.sort_values(['user_id', 'o_date'])
    action_tmp = action[action['day_gap'] < 0]
    action_tmp = action_tmp.sort_values(['user_id', 'a_date'])
    ...
    # train_month = pd.to_datetime(start_date).month
    # train_month_str = str(train_month)
    # # 用户在最后一次购买之后浏览的次数
    # df_label = action_not_buy_101(df_label, order, action, train_month_str, train_month)
    # df_label = action_not_buy_30(df_label, order, action, train_month_str, train_month)
    ...
    # #
    # 最后一次购买到目标的时间差
    df_label = last_day(df_label, order_tmp[(order_tmp['cate'] == 101) | (order_tmp['cate'] == 30)], enddate, 'o_date',
                        'last_buy_101_30')
    df_label = last_day(df_label, order_tmp[(order_tmp['cate'] == 101)], enddate, 'o_date',
                        'last_buy_101')
    df_label = last_day(df_label, order_tmp[order_tmp['cate'] == 30], enddate, 'o_date',
                        'last_buy_30')

    # 最后一次浏览到购买的时间差  效果很差
    df_label = action_last(df_label, action_tmp[(action_tmp['cate'] == 101) | (action_tmp['cate'] == 30)], enddate,
                           'a_date',
                           1, 'last_look_101_30')
    df_label = action_last(df_label, action_tmp[(action_tmp['cate'] == 101) | (action_tmp['cate'] == 30)], enddate,
                           'a_date',
    ...
    df_label = feat_mean(df_label, order_tmp[(order_tmp['cate'] == 30) | (order_tmp['cate'] == 101)], ['user_id'],
                         'o_day', 'day_30_101_mean')
    df_label = feat_mean(df_label, order_tmp[order_tmp['cate'] == 30], ['user_id'], 'o_day', 'day_30_mean')
    df_label = feat_mean(df_label, order_tmp[order_tmp['cate'] == 101], ['user_id'], 'o_day', 'day_101_mean')
    ...
    for i in [7, 14, 30, 90, 180]:
        ...
        df_label = guanxing(df_label, order_tmp[order['cate'] == 101], enddate, o + 'guanx_101')
        df_label = guanxing(df_label, order_tmp[order['cate'] == 30], enddate, o + 'guanx_30')

        # df_label = action_not_buy_101(df_label, order, action, train_month_str, train_month)
        # df_label = action_not_buy_30(df_label, order, action, train_month_str, train_month)
        df_label = feat_mean(df_label, comment[(comment['cate'] == 30) | (comment['cate'] == 101)], ['user_id'],
                             'score_level', o + 'user_comment_score_mean_30_101')
        df_label = feat_mean(df_label, comment[(comment['cate'] == 30)], ['user_id'],
                             'score_level', o + 'user_comment_score_mean_30')
        df_label = feat_mean(df_label, comment[(comment['cate'] == 101)], ['user_id'],
                             'score_level', o + 'user_comment_score_mean_101')
        ...
        df_label = feat_nunique(df_label, order_tmp[order_tmp['cate'] == 83], ['user_id'], 'o_id',
                                o + 'o_id_83_nunique')
        df_label = feat_nunique(df_label, order_tmp[order_tmp['cate'] == 1], ['user_id'], 'o_id',
                                o + 'o_id_1_nunique')
        #######################################################################################################################################
...
def gen_vali(month_4=False):
    train = create_feat('2017-6-1', '2017-7-1', order, action)
    ...
    train.to_csv(r'../input/vali_train_j.csv', index=None)

    test = create_feat('2017-7-1', '2017-8-1', order, action)
    test.to_csv(r'../input/vali_test_j.csv', index=None)

    if month_4:
        train = pd.concat([train, test])
        train.to_csv(r'../input/vali2_train_j.csv', index=None)
        test = create_feat('2017-8-1', '2017-9-1', order, action)
        test.to_csv(r'../input/vali2_test_j.csv', index=None)
...
def gen_test():
    ...
    train = pd.concat([train, create_feat('2017-5-1', '2017-6-1', order, action)])
    train = pd.concat([train, create_feat('2017-4-1', '2017-5-1', order, action)])
    train = pd.concat([train, create_feat('2017-3-1', '2016-4-1', order, action)])
    train = pd.concat([train, create_feat('2017-2-1', '2016-3-1', order, action)])
